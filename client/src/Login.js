import React, {useState, useEffect} from 'react'
import './Login.css'
import { Link, useHistory } from "react-router-dom"; 
import { useStateValue } from './StateProvider.js'
import axios from "./axios.js";
 
function Login() {

    const history = useHistory();
    // dont have null have empty string 
    // the collect typed values 
    const [email, setEmail] = useState('');
    const [password, setPassword] = useState('');
    // Monitors the state of recieved API user data
    const [loggedInUser, setLoggedInUser] = useState('')
    const [{user}, dispatch] = useStateValue(); 
     
    // useEffect hook to execute state change when data comes in from API
    useEffect(() => {
        const {userName, password, email} = loggedInUser;
        console.log(`state is >> ${JSON.stringify(loggedInUser)} `) 
        console.log(` destructured data >> ${userName + ' ' + password +  ' ' +  email} `)
        if (userName && password) {
            dispatch({
                type: "SET_USER",
                user: {
                    userName,
                    email,
                    password,
                }
            });
            history.push('/')
        }
 
      }, [loggedInUser]);
    

    const signIn = async (e) => {
        // dont want refresing 
        e.preventDefault(); 
        console.log('ON CLICK FIRED')
        const response = await axios.get('/', { params: {email,password} })
        const formatResponse = await JSON.stringify(response.data); 
        // Do not try to send stringified data to state 
        const data = await JSON.parse(formatResponse)
        await setLoggedInUser(data)

    }

    const register = e => {
        e.preventDefault(); 
        // register code
    }


    return (
        <div className='login'>

            <Link to="/">
                <img 
                    className="login__logo"
                    src='data:image/png;base64,'></img>
            </Link>

            <div className="login__container">
                <h1>Sign-in</h1>

                <form>
                    <h5>Email</h5>
                    {/* Set value to of input field to what is being set in email state, as it is being entered */}
                    <input type='text' value={email} 
                        onChange={ e => setEmail(e.target.value)}/>

                    <h5>Password</h5>
                    {/* obscures pw charcters with type pw */}
                    <input type='password' value={password} 
                        onChange={ e => setPassword(e.target.value)} />

                    <button 
                        onClick={signIn} 
                        type='submit'
                        className='login__signInButton'
                        >Sign-in
                    </button>
                </form>

                <p> 
                    By signing-in you agree to the AMAZON CLONE Condition of Use & Sale. Please see our non-exising privacy notice, cookies notices, and other stuff. 
                </p>
                 
                 <button 
                    onClick={register}
                    type='submit'
                    className="login__registerButton">Create your Amazon Account
                </button>
            </div>
  
        </div>
    )
}

export default Login
